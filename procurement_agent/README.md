...
1. **Single material category** — The mock catalog only contains cement.
   Real deployments would need multi-category support (steel, sand, etc.).

2. **Static vendor catalog** — Vendors are read from a local JSON file.
   No live pricing, inventory updates, or API integrations.

3. **No authentication / RBAC** — Anyone who can access the CLI can
   approve orders. Production needs role-based access control.

4. **Single-site memory** — `memory_store.json` is a flat file. Concurrent
   writes from multiple agents or sites could cause race conditions.

5. **No in-stock filtering** — `filter_vendors` does not check the
   `in_stock` field. Out-of-stock vendors can still be selected.

6. **No order tracking** — Orders are stored but have no lifecycle
   (pending → shipped → delivered → invoiced).

7. **INR only** — Prices are in Indian Rupees with no
   multi-currency support.

8. **No retry / fallback** — If the Gemini API is unavailable, the agent
   fails. No circuit breaker or graceful degradation.

### Future Improvements

- **Database backend** — Replace JSON files with PostgreSQL or Firestore
  for concurrent access, transactions, and querying.
- **Live vendor APIs** — Integrate with real procurement platforms
  (SAP Ariba, Coupa, IndiaMART) for real-time pricing.
- **Multi-agent orchestration** — Separate agents for sourcing, approval
  routing, and order tracking using ADK sub-agents.
- **Notification system** — Email/Slack alerts for approval requests
  and order confirmations.
- **Analytics dashboard** — Visualise spend by site, vendor performance,
  and approval turnaround times.
- **Multi-language support** — Hindi, Tamil, etc. for site managers.
- **Automated testing with LLM** — Use ADK's evaluation framework to
  test end-to-end agent behaviour with golden scenarios.
